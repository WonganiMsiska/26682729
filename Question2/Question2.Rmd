...
UKweather<- select ()
    ggplot() + 
geom_point(aes(DATE, DX70, size = DX70))  +
    
labs (title = "Sunydays", x = "date", 
    y = "sunnydays per year" )
remove
```
...
```{r}
UKMonthly_Detailed %>% 
  summarise(mean_templess = mean(DX70)) %>% 
  ggplot() + geom_line(aes(DATE, mean_templess), color = "pink", size = 1.2, alpha = 0.8)  + 
  theme_bw() + 
  labs(x = "date", y = "Avg temp", caption = "Calculated using UK weather data",
       title = "UK Avg low temp days")
UKMonthly_Detailed$DATE
```
