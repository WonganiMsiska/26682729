...
## Appendix B {-}
```{r}
> owid.covid.data <- read.csv("~/directory/26682729/Question1/data/owid-covid-data.csv")
>   View(owid.covid.data)
> Deaths_by_cause <- read.csv("~/directory/26682729/Question1/data/Deaths_by_cause.csv")
>   View(Deaths_by_cause)
> covid_data_description <- read.csv("~/directory/26682729/Question1/data/covid_data_description.csv")
>   View(covid_data_description)
```

```{r}
plotcovid<-select(owid.covid.data,location,continent,date,people_fully_vaccinated,total_cases,total_deaths,population) %>% group_by(continent) %>% mutate(casepop= total_cases/population) %>%  mutate(deathpop=total_deaths/population) %>%  mutate(vacpop=people_fully_vaccinated/population) %>% ungroup()
print(plotcovid)
plotcovid[, which( apply( !is.na(plotcovid), 2, all)) ]
```
```{r}
...
h<-plotcovid %>% 
 ggplot() + 
geom_hline(aes(deathpop,continent),fill = "pink", alpha = 0.7) + 
    
coord_flip() + 
labs(x = "deathpop", title = "Total deaths/population in each continent", 
    caption = "from owid_covid_data") 
h
...
```
The findings are quite strange as the 'continent' variable as it is is only 
picking up Africa. We would expect to find more variation in the continents 
